...
- 2026-09-12 — `src/ingest_instruments.py:build()` hardcoded `"status": "current"` and
  ...
  through instead of hardcoding them, renders the whole-part note's reason from
  `cfr_consolidations.PART_REMOVALS` (the record #78 built for the identical sentence in each
  removed section's own document, rather than a third hand-typed copy), and points
  `relationships.related` at the successor rather than at `cited_section_ids()`'s now-equally-
  gone list. The specific "28 audit citations" figure in the old hand-written note is not
  reproduced: no live source in this ingester computes a part-level citation count (only
  `_meta/cited-sections/<part>.yml`'s per-SECTION counts exist, and summing them by hand here
  landed on 27, not 28 — close enough to be worth someone's attention, not close enough to
  republish with confidence), so the regenerated note states the fact generically instead of
  re-asserting a number nothing here can check. `instruments/45-cfr-75.md` is regenerated by
  the fixed ingester and committed as the new baseline; the diff against the hand-written
  original is exactly the wording above plus a YAML line-wrap PyYAML's own `width=100` chose
  for the now-longer title — full text, every other frontmatter field, and all 7 already-split
  section documents are unchanged (`split_cfr_sections.py --check`, `check_extraction.py`,
  `corpus-verify-provenance` all still green). New `--check` flag on `ingest_instruments.py`
  itself (compares every source to what is committed, writes nothing) satisfies the literal
  ask but cannot be CI-wired — a `cfr_part` not already superseded still needs a live
  `cfr_amended_on()` lookup by design (ADR-0001), so plain `--check` here is not hermetic the
  way `split_cfr_sections.py --check` is. The hermetic, CI-wired proof is new
  `src/check_part_supersession.py` (`generated` job), against synthetic fixtures and 45 CFR
  75's real committed snapshot: it also calls `build()` the pre-#77 way (no `status`/
  `superseded_by` arguments) over that same real text and asserts THAT reproduces `status:
  current`, so the fixture is confirmed to fail when the fix it guards is removed. #77
