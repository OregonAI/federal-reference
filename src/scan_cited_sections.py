...
CLAIMS COME FROM ERF'S OWN CATALOG, ONCE, NOT RE-DERIVED FROM RAW TEXT A SECOND TIME.
...
`--audits` IS ACTUALLY SCANNED IN THIS MODE (#64). It was required on the CLI and validated
as a directory since #63, but never read -- ranking and every count came from ERF's catalog
alone, which only ever walks ERF's own document corpus (`scan_external_citations.py`'s own
`content_files()`), never `oregon-audits`. The fix is not to drop the flag; it is to read
it. `oregon-audits` carries no `legal_authority`/`statutes_implemented` concept AT ALL --
measured directly, across all 255 documents in a real checkout: exactly one file carries
either field, `_meta/templates/document.md`, and its `legal_authority` value is the empty
...
known (title, part) and cannot be reused for a scan whose whole point is not knowing the
part yet. `AUDIT_CFR_RE` mirrors the SHAPE of ERF's own `FED` pattern in
`scan_external_citations.py` instead -- title digits, a CFR token tolerant of punctuation,
an optional `Part`/`§` marker, a part number with an optional letter suffix (`7 CFR 1c`,
`8 CFR 274a`) -- rather than inventing a fresh one. Its counts are merged ADDITIVELY into a
catalog row's own `mentions` (a part both sources cite is not double-scanned, just summed),
and a part audits cite that the catalog never does at all becomes a NEW row at
`authority_claims: 0` -- the same 42 CFR Part 2 shape (0 claims, 39 mentions) #63 already
put in this queue on purpose, so a human can see the case the ranking cannot make. A HELD
part is excluded from the queue no matter which source names it: 2 CFR 200 is cited 100+
times in oregon-audits and is held, so it never reaches the queue -- its mentions have
nowhere to go but the discarded catalog-held bucket, same as any other held part's would.
Every row's `cited_in` names which source(s) (`erf`, `audits`) contributed its `mentions`,
mirroring the per-part mode's own `cited_in` above, so "0 claims, 8 mentions" reads as
"auditors, not rules" rather than looking like an uncited guess.
...
# A GENERAL CFR-part regex for AUDIT MENTIONS (#64) -- distinct from patterns() above,
# which is anchored to a known (title, part) and cannot answer "which parts does this
# corpus mention" without one. Mirrors the SHAPE of ERF's own `FED` pattern in
# scan_external_citations.py (title digits, a CFR token tolerant of "C.F.R.", "CFR",
# "C F R", an optional "Part"/section-mark, a part number with an optional letter suffix
...
def scan_audit_mentions(audits: pathlib.Path) -> tuple[collections.Counter, int]:
    ...
    trusts. Same SKIP_PARTS and sorted walk as scan() above, for the same reproducibility
    ...
    for path in sorted(audits.rglob("*.md")):
        ...
        n += 1
        text = path.read_text(errors="ignore")
        for m in AUDIT_CFR_RE.finditer(text):
...
def rank_targets(targets: list[dict], held: set[tuple[str, str]],
                  audit_mentions: dict[tuple[str, str], int] | None = None) -> list[dict]:
    ...
    `audit_mentions` -- {(title, part): count} from scan_audit_mentions() (#64) -- merges
    ADDITIVELY into a catalog row's own `mentions`, and licenses a NEW row (authority_claims
    always 0, since audits carry none -- see the module docstring) for a part audits cite
    that ERF's catalog never does at all. `held` is checked for BOTH kinds of row: a held
    ...
    # Same held-check as above; authority_claims is always 0 (#64: audits carry none).
    for (title, part), audit_n in audit_mentions.items():
        if (title, part) in catalog_keys or (title, part) in held:
            continue
        rows.append({
            "part_id": f"{title}-cfr-{part}",
            "citation": f"{title} CFR {part}",
...
def queue_header() -> list[str]:
    ...
    return [
        ...
        "#",
        "# MENTIONS ARE MERGED WITH A REAL oregon-audits SCAN (#64): a row's `mentions` is",
        "# ERF's own catalog count PLUS body-text CFR-part occurrences scanned directly from",
        "# oregon-audits, added together -- never a second claims count, since audits carry",
        "# no legal_authority/statutes_implemented at all (measured, see the module",
        "# docstring) and so cannot move the claims ranking. `audit_only_parts` counts rows",
        "# below that exist ONLY because oregon-audits cites them -- absent from ERF's",
        "# catalog entirely, held-checked the same as every other row, always at",
        "# authority_claims: 0. unheld_parts = (scanned_targets - held_parts) +",
...
def build_queue_lines(catalog: dict, held: set[tuple[str, str]],
                       audit_mentions: dict[tuple[str, str], int] | None = None
                       ) -> tuple[list[str], int, int, int]:
    """Compute `_meta/ingest-queue.yml`'s content -- PURE, no I/O. Returns (lines,
    unheld_parts, total_cfr_targets, audit_only_parts) so the caller can decide whether to
    write it and what to report. Raises nothing; a catalog with zero CFR targets is the
    caller's refusal to make, not this function's (see the module docstring: zero found is
    a REFUSAL, not an empty queue) -- and specifically NOT this function's job to act on by
    touching `QUEUE_OUT`, which is why this function touches no path at all.

    `audit_mentions` -- {(title, part): count} from scan_audit_mentions() (#64) -- is
    passed straight through to rank_targets(); `total_cfr_targets` (`scanned_targets` in
    the written file) deliberately stays the CFR-shaped subset of ERF's catalog ALONE, not
    inflated by audit-only rows, so `unheld_parts` needs the separate `audit_only_parts`
    return value to reconcile against it (see queue_header())."""
    audit_mentions = audit_mentions or {}
    ...
    ranked = rank_targets(targets, held, audit_mentions)
    audit_only = [r for r in ranked if r["cited_in"] == ["audits"]]
...
def discover_main(args: argparse.Namespace) -> int:
    ...
    #63/#64 sections for why claims come from ERF's own catalog while mentions are that
    catalog's count merged with a real oregon-audits scan."""
    ...
    audit_mentions, audit_files = scan_audit_mentions(args.audits)
    ...
    if unheld_n == 0:
        ...
        return 1
    write_queue(lines)
    print(f"  wrote {QUEUE_OUT.relative_to(ROOT)}: {unheld_n} queue entries "
          f"({unheld_n - audit_only_n} unheld of {total_n} catalog CFR parts + "
          f"{audit_only_n} audit-only)")
    return 0
...
def check_queue() -> int:
    ...
    WHAT THIS DELIBERATELY DOES NOT DO: re-read ERF's catalog, re-scan oregon-audits, or
    re-rank from either. That would need `--erf`/`--audits` pointed at real sibling
    checkouts, which is exactly what CI cannot do (see the module docstring's WHY COMMIT
    section) -- a `--check` that tried would either fail on every PR for a reason that is
    not drift, or silently skip itself in CI and verify nothing while its name claims it
